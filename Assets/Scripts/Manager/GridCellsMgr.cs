...
public class GridCellsMgr : MonoBehaviour
{
    ...
    public void LoadFogs()
    {
        if (GameMgr.Instance.isLandScape)
        {
            for (int i = 4; i <= 8; i++)
            {
                for (int j = 0; j <= 3; j++)
                {
                    if (!(i >= 5 && i <= 7 && j >= 0 && j <= 1) && !((i == 4 && j == 3) || (i == 5 && j == 3) || (i == 7 && j == 3) || (i == 8 && j == 3)))
                    {
        ...
        else
        {
            for (int i = 1; i <= 4; i++)
            {
                for (int j = 0; j <= 3; j++)
                {
                    if ((i <= 2 && i >= 1 && j <= 1 && j >= 0) || (i == 3 && j == 0) || (j == 3 && i != 3)) continue;
                    GameObject temp = SpawnEntitiesOffGrid("Fog_Cloud", i, j);
                    Fog fog = temp.GetComponent<Fog>();
                    fog.indexX = i; fog.indexY = j;
                    GameMgr.Instance.fogs_0.Add(fog);
    ...
    public void LoadEntities()
    {
        if (GameMgr.Instance.isLandScape)
        {
            LoadEntitiesCellFromMapDataPrincess("Seed_FruitTree", 6, 0);
            LoadEntitiesCellFromMapDataPrincess("Seed_FruitTree", 7, 0);
            LoadEntitiesCellFromMapDataPrincess("Seed_FruitTree", 5, 1);
        }
        else
        {
            LoadEntitiesCellFromMapDataPrincess("Seed_FruitTree", 2, 0);
            LoadEntitiesCellFromMapDataPrincess("Seed_FruitTree", 1, 1);
            LoadEntitiesCellFromMapDataPrincess("Seed_FruitTree", 3, 0);
        }
