...
public class GridCellsMgr : MonoBehaviour
{
    ...
    public void LoadEntities()
    {
        if (GameMgr.Instance.isLandScape)
        {
            LoadEntitiesCellFromMapDataPrincess("MonsterHouse_6", 6, 1);
        }
        else
        {
            LoadEntitiesCellFromMapDataPrincess("MonsterHouse_6", 3, 1);
        }
    ...
    public GameObject SpawnEntitiesOffGrid(string entityName, int xCellIndex, int yCellIndex)
    {
        ...
        else
        {
            ...
            {
                // The tree sprite uses a bottom pivot; align its trunk with the desert tile.
