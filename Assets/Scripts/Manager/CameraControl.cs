...
public class CameraControl : MonoBehaviour
{
    ...
    {
        GameMgr.Instance.isCameraZooming = true;
        float targetAspect = 16f / 9f; // 目标宽高比
        float currentAspect = (float)Screen.width / Screen.height;
        var _targetSize = targetSize * (targetAspect / currentAspect);

        if (currentAspect < targetAspect) // 比目标更窄（如手机竖屏）
        {
            if (_targetSize >= 6f) _targetSize = 6f;
        }
        else // 比目标更宽（如超宽屏）
        {
            _targetSize = targetSize; // 默认大小
        }


        StartCoroutine(ZoomCoroutine(_targetSize, zoomTime, () =>
        {
            GameMgr.Instance.basicSize = targetSize;
            GameMgr.Instance.isCameraZooming = false;
        }));
    }
