...
{
    public GameObject GetTarget() { return this.gameObject; }
    ...
    public static int CountAnyWithStringInFunctionCallName(GameObject target, string toCheck)
	{
	...
	public static void StopAllExceptForName(GameObject target, string nameToLeavePlaying)
	{
		Component[] components = target.GetComponents<iTween>();
		foreach (iTween iTween in components)
		{
			if (iTween._name != nameToLeavePlaying)
			{
				iTween.Dispose();
			}
		}
	...
	private void GenerateShakePositionTargets()
	{
		...
		if (this.isLocal)
		{
			this.vector3s[0] = base.transform.localPosition;
		}
		else
		{
	...
	{
		UnityEngine.Debug.LogError("CameraFade is no longer supported in iTween!");
	}
	...
	public static int Count(GameObject target)
	{
		...
		{
			int result = 0;
			iTween.tweenCount.TryGetValue(target.GetInstanceID(), out result);
			return result;
		}
		Component[] components = target.GetComponents<iTween>();
	...
	private void OnDestroy()
	{
		...
		int instanceID_0 = base.gameObject.GetInstanceID();
		if (iTween.tweenCount.ContainsKey(instanceID_0))
		{
			int num = iTween.tweenCount[instanceID_0];
			int num2 = num - 1;
			if (num2 == 0)
			{
				iTween.tweenCount.Remove(instanceID_0);
			}
			else
			{
				iTween.tweenCount[instanceID_0] = num2;
			}
		}
	}
	...
	private void Awake()
	{
		...
		if (iTween.OPTIMIZED)
		{
			...
			if (!iTween.tweenCount.ContainsKey(instanceID_0))
			{
				iTween.tweenCount.Add(instanceID_0, 0);
			}
			Dictionary<int, int> dictionary =new Dictionary<int, int>();
			int key = 0;
			(dictionary = iTween.tweenCount)[key = instanceID_0] = dictionary[key] + 1;
		}
	}
	...
	private static readonly Dictionary<int, int> tweenCount = new Dictionary<int, int>(1024);
